...
if __name__ == '__main__':
    ...
    for pref_a in prefs:
        for pref_b in prefs:
            ...
            pref1 = np.array([0.005, 0.025, pref_a, 0.97 - pref_a], dtype=np.float32)
            pref2 = np.array([0.005, 0.025, pref_b, 0.97 - pref_b], dtype=np.float32)#np.array([0.005, 0.025, 0.97/2, 0.97/2], dtype=np.float32) # ps.sample() #pref1.copy() # Maybe change to separate preferences later
            ...
            results.append([pref1[2], pref2[2], lone1_captures, lone2_captures, team_captures])

            
    results = pd.DataFrame(results, columns = ["Competitive 1", "Competitive 2", "Lone 1 Capture", "Lone 2 Capture", "Team Capture"])
    
    results.to_csv(f'{PATH_DIR}/wolfpack/wolfpack_tuning_varied_prefs_DDMMYY.csv')
