...
class Gridworld(Env):
    ...
    def __initialise_grid_display(self, boundaries):
        ...
        """
        # Set axis limits
        self.ax.set_xlim(-0.5, self.columns - 0.5)
        ...
        self.ax.set_xticks(np.arange(-.5, self.columns, 1))
        self.ax.set_yticks(np.arange(-.5, self.rows, 1))
        ...
        # Display image
        self.image = self.ax.imshow(self.create_image_observation())
        
    def render(self, boundaries=False):
        """
        ...
        # self.image.set_data(self.create_image_observation())
        # #plt.draw()
        ...
        plt.figure(1)
        plt.clf()
        plt.imshow(self.create_image_observation())
        plt.axis('off')
        plt.pause(0.00001)
